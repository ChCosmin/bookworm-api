        ...
        res.json({ 
          books: goodreadsResult.GoodreadsResponse.search[0].results[0].work.map(
            work => ({
              ...
              thumbnail: [work.best_book[0].small_image_url[0]]
            })
          )
        })
      )
...
router.get("/fetchBookInfo", (req, res) => {
  ...
  request
    ...
    .then(result => 
      parseString(result, (err, goodreadsResult) => {
        ...
        const descriptionText = goodreadsResult.GoodreadsResponse.book[0].description[0];
        const description = descriptionText !== '' ? descriptionText : "No description"; 
        
        const linkToBook = goodreadsResult.GoodreadsResponse.book[0].link[0];
